...
0.15.0 builds its own MoltenVK instead of taking the one from Homebrew, with four fixes on top of the pinned upstream, all in patches/moltenvk/. Long games could still lose the device once in a while: something freed a buffer while the GPU was still reading it, and Metal has no safety net for that the way the Linux kernel does. The command buffer now holds its resources until the GPU is finished with them. Across every run that killed the stock driver, the patched one stayed up, and the frame rate is the same. The build-progress pie drew as a diagonal seam instead of a filling circle, because MoltenVK dropped the triangle-fan shape when the game set it on the fly and drew a plain triangle list from the same points. Saving a game and continuing could crash in a Metal callback, a query pool freed before its completion handler ran. And if the device is lost anyway, the game quits to the menu now instead of drawing on a dead GPU until macOS runs out of memory and needs a reboot.
